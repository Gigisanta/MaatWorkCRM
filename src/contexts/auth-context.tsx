...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string, rememberMe = false) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || ERROR_MESSAGES.LOGIN_ERROR);
      }
  ...
  const register = useCallback(async (data: RegisterData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || ERROR_MESSAGES.REGISTER_ERROR);
      }
      ...
      const message = error instanceof Error ? error.message : ERROR_MESSAGES.REGISTER_ERROR;
      toast.error(message);
      throw error;
    }
  }, [router, notifyOtherTabs]);
