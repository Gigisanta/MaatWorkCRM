...
export const getConversionFunnel = createServerFn({ method: "GET" })
  .inputValidator((input: { orgId: string }) => input)
  .handler(async ({ data }) => {
    const contactsPerStageRaw = await db
      .select({
        stageId: pipelineStages.id,
        stageName: pipelineStages.name,
        stageOrder: pipelineStages.order,
    ...
    const contactsPerStage = contactsPerStageRaw.map((stage) => ({
      ...stage,
      count: Number(stage.count),
    }));

    const stages = contactsPerStage; // for the clientStage logic
    ...
    const funnelData = contactsPerStage.map((stage, idx) => {
      ...
      const _stageInfo = stages.find((s) => s.stageId === stage.stageId);
      return {
        stageId: stage.stageId,
        stageName: stage.stageName,
        count: stage.count,
