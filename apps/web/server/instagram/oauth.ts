...
export async function exchangeCodeForToken(
  ...
  const response = await fetch(
    `https://api.facebook.com/v18.0/oauth/access_token?${params.toString()}`
  );
...
export async function getInstagramPage(
  ...
  const response = await fetch(
    `https://graph.facebook.com/v18.0/${userId}/accounts?${params.toString()}`
  );
  ...
  for (const page of data.data || []) {
    ...
    const pageResponse = await fetch(
      `https://graph.facebook.com/v18.0/${page.id}?${pageParams.toString()}`
    );
