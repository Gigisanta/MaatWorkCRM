...
function CareerProgressBar() {
  ...
  return (
    // biome-ignore lint/a11y/useSemanticElements: Cannot use semantic button due to nested button elements
    <div
      onClick={() => (window.location.href = "/career-plan")}
      onKeyDown={(e) => {
        if (e.key === "Enter") window.location.href = "/career-plan";
      }}
      role="button"
      tabIndex={0}
      className="hidden lg:flex flex-col items-start gap-1.5 px-4 py-2 rounded-lg bg-surface-100/50 hover:bg-surface-100 border border-border/30 transition-all duration-150 group cursor-pointer min-w-[200px] text-left"
    >
              ...
              {recentNotifications.length === 0 ? (
                ...
                recentNotifications.map((n) => (
                  <button
                    ...
                    className={cn(
                      "p-3 rounded-lg cursor-pointer transition-all hover:bg-surface-hover text-left w-full",
