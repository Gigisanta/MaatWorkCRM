...
import { type NotifType, type Notification, type Priority, useNotifications } from "../../hooks/useNotifications";
...
export const NotificationCenter: React.FC = () => {
    ...
    return (
      <div key={type} className="mb-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs rounded px-2 py-0.5" style={{ background: color, color: "white" }}>
            {" "}
            {items.length}{" "}
          </span>
        </div>
        <div className="flex flex-col space-y-1">
          {items.map((n: Notification) => (
            <div
              key={n.id}
              className="flex items-center justify-between p-3 bg-white/5 rounded-lg hover:bg-white/8 cursor-pointer"
              onClick={() => openDetail(n)}
            >
              <div className="flex items-center gap-3 min-w-0">
                <span className="h-2 w-2 rounded-full" style={{ background: priorityColor(n.priority) }} />
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium truncate${n.read ? " text-slate-400" : " text-white"}`}>
                    {n.message}
                  </div>
              ...
              <div className="flex items-center gap-2">
                {!n.read && (
                  <span className="text-xs font-semibold" style={{ color: priorityColor(n.priority) }}>
                    •
                  </span>
                )}
                <span
          ...
          <div className="flex items-start justify-between mb-3">
            ...
            <Dialog.Close asChild>
              <button
                ...
                className="text-lg font-bold leading-none rounded focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
              >
                ×
              </button>
            </Dialog.Close>
          ...
          {detailNotification ? (
            <div className="flex flex-col space-y-2">
              <div className="text-sm text-slate-100">
                Type: <span className="font-semibold">{detailNotification.type}</span>
              </div>
              <div className="text-sm text-slate-100">
                Priority:{" "}
                <span className="font-semibold" style={{ color: priorityColor(detailNotification.priority) }}>
                  {detailNotification.priority}
                </span>
              </div>
              <div className="text-sm text-slate-100">
                Time: {new Date(detailNotification.timestamp).toLocaleString()}
              </div>
              ...
              <div className="flex gap-2 pt-2 mt-2 border-t border-white/10">
                {!detailNotification.read && (
                  <button
                    className="px-3 py-1 rounded bg-green-600/80 hover:bg-green-600 text-white"
                    onClick={() => {
                      updateNotification(detailNotification.id, { read: true });
                      setDetailNotification((d) => (d ? { ...d, read: true } : d));
                    }}
                  >
                    Mark Read
                  </button>
                ...
                <button
                  ...
                  onClick={() => {
                    handleSnooze(detailNotification.id, 5);
                  }}
                >
                  Snooze 5m
                </button>
                <button
                  ...
                  onClick={() => {
                    handleDismiss(detailNotification.id);
                  }}
                >
                  Dismiss
                </button>
  ...
  return (
    <div className="inline-block">
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Trigger asChild>
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-full bg-black/40 hover:bg-black/60 shadow-md"
          >
          ...
          <Dialog.Content className="fixed right-0 top-0 h-full w-96 bg-neutral-900 shadow-lg p-4 m-4 rounded-lg overflow-auto">
            <div className="flex items-center justify-between mb-3">
              ...
              <Dialog.Close asChild>
                <button
                  ...
                  className="p-1 rounded bg-white/10 hover:bg-white/20 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
                >
                  Close
                </button>
              </Dialog.Close>
            ...
            <div className="mb-2 border-b border-white/10 pb-2 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                {(["All", "alerts", "system", "actions"] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => setGroup(t as GroupTabs)}
                    className={`text-sm px-2 py-1 rounded ${group === t ? "bg-white/10" : "bg-transparent"}`}
                  >
            ...
            <div className="space-y-3 pr-2" style={{ maxHeight: "70vh" }}>
              {group === "All"
                ...
                : renderGroupList(
                    [group as NotifType],
                    byGroup(notifications, group as NotifType),
                    openDetail,
                    priorityColor,
                  )}
            </div>
      ...
      {toast && (
        <div
          className="fixed bottom-4 right-4 bg-neutral-800/90 text-white text-sm px-4 py-2 rounded-lg shadow-lg"
          role="status"
          aria-live="polite"
        >
          {toast.message}
...
function renderGroupList(
  types: (NotifType | string)[],
  items: Notification[],
  openDetail: (n: Notification) => void,
  colorFn: (p: Priority) => string,
) {
