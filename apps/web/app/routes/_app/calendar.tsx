...
import { Plus, ChevronLeft, ChevronRight, Video, Phone, MapPin, Calendar as CalendarIcon, Clock, MoreHorizontal } from "lucide-react";
...
const DEMO_EVENTS = [
  { id: "1", title: "Reunión María López", type: "meeting", startAt: "10:00", endAt: "11:00", day: 4, location: "Oficina", color: "indigo" },
  { id: "2", title: "Llamada Juan Martínez", type: "call", startAt: "14:00", endAt: "14:30", day: 5, color: "emerald" },
  { id: "3", title: "Equipo semanal", type: "meeting", startAt: "09:00", endAt: "10:00", day: 7, location: "Zoom", color: "amber" },
  { id: "4", title: "Follow-up Roberto", type: "call", startAt: "16:00", endAt: "16:30", day: 10, color: "violet" },
  { id: "5", title: "Presentación propuesta", type: "meeting", startAt: "11:00", endAt: "12:00", day: 11, location: "Cliente", color: "blue" },
];
        ...
        <div className="lg:col-span-3 space-y-4">
          <Card variant="glass" className="overflow-hidden border-secondary/10">
            <CardHeader className="bg-secondary/5 border-b border-secondary/10 px-6 py-4">
              <Stack direction="row" align="center" justify="between">
                <h2 className="text-xl font-bold text-text capitalize">{monthName} {year}</h2>
                <Stack direction="row" gap="xs">
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                    <ChevronLeft size={18} />
                  </Button>
                  <Button variant="secondary" size="sm" className="h-8 rounded-full text-xs font-bold uppercase tracking-wider">Hoy</Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
            ...
            <CardContent className="p-0">
              <div className="grid grid-cols-7 border-b border-secondary/5">
                {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map((d) => (
                  <div key={d} className="py-3 text-center text-[10px] font-black text-text-muted uppercase tracking-widest bg-secondary/5">
                    {d}
                  </div>
                ))}
              </div>
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span className={cn(
                          ...
                          isToday ? "bg-primary text-white shadow-lg scale-110" : "text-text-secondary group-hover:text-text"
                        )}>
                          {day}
                        </span>
                        {dayEvents.length > 0 && (
              ...
              <div className="space-y-2">
                ...
                <div className="pt-2 border-t border-primary/10">
                  <p className="text-[10px] text-text-muted leading-relaxed">
                    Tu agenda está un <span className="text-primary font-bold">15% más ocupada</span> que la semana pasada.
                  </p>
                </div>
              </div>
            </CardContent>
