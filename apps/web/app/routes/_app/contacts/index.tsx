...
function ContactsPage() {
  ...
  // ⚡ Bolt Optimization:
  // Memoizing columns with an empty dependency array [] prevents @tanstack/react-table
  // from completely tearing down and rebuilding its internal column model on every
  // component render or dynamic state change.
  // We pass dynamic row-specific state (like openDropdownId) and handlers via the
  // `meta` object to `<DataTable />` and access them via `info.table.options.meta`.
    ...
    () => [
      {
        ...
        cell: (info: any) => {
          ...
          const isEditingEmoji = meta?.editingEmojiId === contact.id;
          return (
            <div className="flex items-center gap-4">
              <div className="relative">
                {isEditingEmoji ? (
                  <AvatarPicker
                    value={contact.emoji || "👤"}
                    onChange={(emoji) => meta?.handleEmojiChange?.(contact.id, emoji)}
                    size="md"
                  />
                ) : (
                  <button
                    onClick={() => meta?.setEditingEmojiId?.(contact.id)}
